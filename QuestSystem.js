/*
プラグインパラメーター周りの日本語を追加

追加改善案（自分のスキルでやると怪しいので触れなかった）
1.アイコンIDの入力欄はnumberにすると右クリックで出る「アイコンビューア」が使えないので、stringにして、コード側で整数に変換とすると、想定外のミスでもエラーが出ないで済むかも？
2.@param MenuCommandsのクエストコマンドの入力をselectタイプにできないか？手法が分かりませんでした。
*/
