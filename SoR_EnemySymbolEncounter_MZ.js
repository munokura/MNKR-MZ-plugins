...
// ----------------------------------------------------------------------------
// Latest version v1.11 (2020/09/13)
//=============================================================================
/*:
 * @plugindesc ＜シンボルエンカウント総合＞ v1.11m
 * @author 蒼竜 （改変：ムノクラ）
 ...
 * @text 通常エンカウント無効化
 * @desc true: 通常エンカウント方式の無効化設定をスクリプトが代行します (ツクールデフォルト: false)
 * @type boolean
 * @default false
 * @on 無効化
 * @off 有効化
...
Game_Event.prototype.AdditionalSerachForFollowers = function(e) {
	...
	for(let i=0; i<n_flw;i++){
		...
		const bush_rate = $gameMap.isBush(p.x,p.y) == true ? this.bushrate:1;
		 dist = calcDist(e,p);
		if(dist <= Math.ceil(this.see_distance*bush_rate)){
			var ang = calcAngle(e,p);
			if(ang >= -this.see_angle && ang <= this.see_angle){			 
