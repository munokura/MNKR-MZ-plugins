...
(() => {
  ...
  const parameters = PluginManager.parameters('MNKR_TMTimerExMZ');
  const timerX = Number(parameters['timerX'] || 816);
  const timerY = Number(parameters['timerY'] || 48);
  const showMinutes = eval(parameters['showMinutes'] || true);
  const msecShiftY = Number(parameters['msecShiftY'] || 0);
  const pluginName = "MNKR_TMTimerExMZ";
  ...
  PluginManager.registerCommand(pluginName, "timerMsec", args => {
    ...
    if (variableId) {
      $gameVariables.setValue(variableId, $gameTimer.msec());
    }
  });
