...
Extend the timer functionality simply by installing this plugin.

When using the timer judgment of the "Conditional Branch" event command,
it may be determined that the timer has run out even if there is still time
remaining in milliseconds.

This problem can be avoided by assigning the remaining time to a game variable
using the plugin command,
and then using that variable to execute the conditional branch.

Displaying milliseconds requires redrawing every frame, which can slow down
processing.

Plugin Command:

timerMsec 5
Assigns the remaining timer time in milliseconds to game variable 5.
...
MIT License.
http://opensource.org/licenses/mit-license.php
You may modify and redistribute it without permission from the author,
and there are no restrictions on its use (commercial, R18, etc.).

@param timerX
@text Timer X coordinate
@desc The X coordinate to display the timer.
@type number
@default 816

@param timerY
@text Timer Y coordinate
@desc The Y coordinate at which to display the timer.
@type number
@default 48

@param showMinutes
@text Timer display format
@desc Displays the timer in minutes:seconds format.
@type boolean
@on minutes: seconds
@off Seconds only
@default true
...
@param msecShiftY
@text Y adjustment in milliseconds
@desc Adjusts the Y coordinate of the millisecond display.
@type number
@default 36

@command timerMsec
@text Assign the remaining time to the timer
@desc Sets the remaining time on the timer.
@arg variableId
@text Variable ID
@desc Sets the variable to the number of milliseconds remaining on the timer.
@type variable
@default 0
*/
