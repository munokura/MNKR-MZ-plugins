/*
 * --------------------------------------------------
 * MNKR_TMCharacterExMZ Ver.1.0.0
...
(() => {
    ...
    "use strict";
    const pluginName = 'MNKR_TMCharacterExMZ';
    const parameters = PluginManager.parameters(pluginName);
    const LandingAnimation = eval(parameters['landingAnimation'] || 'false');
    ...
    Game_CharacterBase.prototype.updateJump = function() {
        ...
        if (LandingAnimation && this._jumpCount === 0) {
    ...
    PluginManager.registerCommand(pluginName, "chrScale", function(args) {
        ...
        const character = this.character(Number(args.eventId));
        if (character) character.setChrScale(scaleX, scaleY);
    });
